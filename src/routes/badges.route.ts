...
router.post(
    ...
    async (req: Request, res: Response) => {
        ...
        await helperDatabase
            ...
            .then(() => {
                logger.log('success', `Badge ${badgeInstance.uuid as string} added`);
                res.json({
                    badgeInstance
                });
            })
...
router.put('/edit/badge/:uuid', authKey, async (req: Request, res: Response) => {
    ...
    await helperDatabase.editBadge(masterInstance, uuid, updatedBadge).then(() => {
        res.json({
            updatedBadge
        });
    }
...
router.delete('/remove/badge/:uuid', authKey, async (req: Request, res: Response) => {
    const { uuid } = req.params;
    ...
    await helperDatabase.removeBadge(masterInstance, uuid).then(() => {
...
router.get('/fetch/badge/:uuid', authKey, async (req: Request, res: Response) => {
    const { uuid } = req.params;
...
router.post('/add/user',
    ...
    authKey, async (req: Request, res: Response) => {
        ...
        await helperDatabase.addBadgeToUser(masterInstance, user as string, badge as string).then(() => {
            logger.log('success', `Badge ${badge as string} added to user ${user as string}`);
            res.json({
                message: `Badge ${badge as string} added to user ${user as string}`
            });
        }
        ).catch(() => {
            res.status(500).json({ error: 'Internal Server Error' });
...
router.delete('/remove/user',    
    ...
    authKey, async (req: Request, res: Response) => {
        ...
        await helperDatabase.removeBadgeFromUser(masterInstance, user as string, badge as string).then(() => {
            logger.log('success', `Badge ${badge as string} removed from user ${user as string}`);
            res.json({
                message: `Badge ${badge as string} removed from user ${user as string}`
            });
        }
        ).catch(() => {
            res.status(500).json({ error: 'Internal Server Error' });
